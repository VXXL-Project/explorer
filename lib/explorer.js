...
module.exports = {
    ...
    if (settings.use_rpc) {
      if (settings.nethash == 'netmhashps') {
        rpcCommand([{method:'getmininginfo', parameters: []}], function(response){
          ...
          if (response.networkghps) {
            response.netmhashps = parseFloat(response.networkghps);
  ...
  get_input_addresses: function(input, vout, cb) {
    ...
    if (input.coinbase) {
      ...
      if (settings.constantInputAddresses[input.txid]) {
        return cb(settings.constantInputAddresses[input.txid]);
      }
