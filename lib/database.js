...
function update_address(hash, blockheight, txid, amount, type, data, cb) {
  var addr_inc = {}
  if (hash == 'coinbase') {
  ...
  find_address(hash, function(address) {
    if (address) {
      Address.findOneAndUpdate({a_id: hash}, { $inc: addr_inc }, { new: true, upsert: true }, function (err, address) {
...
function save_tx(txid, blockheight, blocktime, cb) {
  ...
  lib.get_rawtransaction(txid, function(tx){
    if (tx != 'There was an error. Check your console.') {
      ...
      lib.syncLoop(tx.vin.length, function (vinloop) {
        ...
        remove_unspent(tx.vin[n].txid, tx.vin[n].vout, function() {
          vinloop.next();
        })
      }, function() {
        lib.syncLoop(tx.vout.length, function (voutloop) {
          ...
          if (tx.vout[m].scriptPubKey.addresses && tx.vout[m].scriptPubKey.addresses[0]) {
            add_unspent(tx.vout[m].scriptPubKey.addresses[0], txid, tx.vout[m], function(){
              voutloop.next();
            });
          } else {
            voutloop.next();
      ...
      lib.prepare_vin(tx, function(vin) {
        lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
          lib.syncLoop(vin.length, function (loop) {
            ...
            update_address(nvin[i].addresses, blockheight, txid, nvin[i].amount, 'vin', tx.vin[i] ,function(){
              loop.next();
            });
          }, function(){
            lib.syncLoop(vout.length, function (subloop) {
              ...
              if (vout[t].addresses) {
                update_address(vout[t].addresses, blockheight, txid, vout[t].amount, 'vout', tx.vout[t], function(){
                  subloop.next();
                });
              } else {
                subloop.next();
              ...
              lib.calculate_total(vout, function(total){
                var newTx = new Tx({
                  ...
                  timestamp: tx.time ? tx.time : blocktime,
...
function remove_unspent(txid, vout, cb) {
  Address.find({'unspent':{$elemMatch: {'txid': txid, 'vout': vout}}}).exec(function(err, addresses){
    if (err || !addresses.length) return cb(err)
      
    const unspent_list = addresses[0].unspent.filter(item => !(item.txid === txid && item.vout === vout));

    Address.updateOne({a_id:addresses[0].a_id}, { unspent: unspent_list }, function() {
      return cb();
    });
  });
}

function add_unspent(hash, txid, unspent_data, cb) {
  find_address(hash, function(address) {
    if (address) {
      
      address.unspent.push({
        txid: txid, 
        vout: unspent_data.n, 
        scriptPubKey: unspent_data.scriptPubKey.hex, 
        satoshis: (unspent_data.valueSat)?unspent_data.valueSat:unspent_data.value*100000000 
      });

      Address.updateOne({a_id:hash}, { unspent: address.unspent }, function() {
        return cb();
      });
    }
    return cb();
  ...
  create_txs: function(block, cb) {
    is_locked("db_index", function (exists) {
      if (exists) {
        ...
        lib.syncLoop(block.tx.length, function (loop) {
          var i = loop.iteration();
          save_tx(block.tx[i], block.height, block.time, function(){
              loop.next();
          });
        });
      }
                    ...
                    async.eachLimit(block.tx, task_limit_txs, function(txid, next_tx) {
                      Tx.findOne({txid: txid}, function(err, tx) {
                        if(tx) {
                          ...
                          save_tx(txid, block_height, block.time, function(err){
                            if (err) {
                              console.log(err);
                            } else {
