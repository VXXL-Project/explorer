          ...
          if (hash != 'coinbase') {
            ...
            AddressTx.findOneAndUpdate({ a_id: hash, txid: txid }, {
              ...
              if (err) {
                ...
                var hasUnspent = address.find(item => item.txid === txid)

                if (!hasUnspent) {
                  address.unspent.push({ txid: txid, vout: data.n, scriptPubKey: data.scriptPubKey.hex, satoshis: (data.valueSat)?data.valueSat:data.value*100000000 })
                }

                lib.is_unique(address.txs, txid, function(unique, index) {
                  if (unique == false) return

                  address.txs.push({ addresses: txid, type: type });

                  if (address.txs.length > settings.txcount ) {
                    address.txs.shift();
                  }
                })
...
function remove_from_unspent(txid, index, cb)
{
  Address.find({'unspent':{$elemMatch: {'txid':txid, 'vout':index}}}).exec(function(err, addresses){
    if (err || addresses.length!=1) {
      ...
      Address.updateOne({a_id:addresses[0].a_id}, {
        unspent: arr_unspent,
      }, function() {
        return cb();
      });
  ...
  create_txs: function(block, cb) {
    ...
    is_locked("db_index", function (exists) {
      if (exists) {
        ...
        lib.syncLoop(block.tx.length, function (loop) {
          var i = loop.iteration();
          save_tx(block.tx[i], block.height, block.time, function(tx_unspent, err){
                    ...
                    async.eachLimit(block.tx, task_limit_txs, function(txid, next_tx) {
                      Tx.findOne({txid: txid}, function(err, tx) {
                        if(tx) {
                          ...
                          save_tx(txid, block_height, block.time, function(tx_unspent, err){
                            if (err) {
                              console.log(err);
                            } else {
                              Array.prototype.push.apply(block_unspent,tx_unspent)
